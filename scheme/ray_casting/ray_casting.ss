...
;;Visited cells is a nice little list of the cells we've already cehcked to speed things up
    ...
    (lambda (env char-x char-y x y dx dy i visited-cells)
        (if (not (> i VIEW-RADIUS))
            (let* ((cell-x (inexact->exact (round dx)))
                ...
                (if cell                                        ;;Check to make sure we got a cell
                    (if (not (member coords visited-cells))     ;;Check to make sure we haven't touched it yet
                        (let ((new-visited-cells (cons coords visited-cells)))
                            (set-cell-visible cell #t)
                            (if (cell-opaque? env cell-x cell-y) ;;if it blocks vision we don't want to continue
                                new-visited-cells
                    ...
                    visited-cells))
            visited-cells))) ;;Return the cells we touched back to the parent
